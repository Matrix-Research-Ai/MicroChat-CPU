...
orig_model = model
if device_type != "cuda":
    print0("CPU mode: skipping torch.compile (no MSVC compiler needed)")
else:
...
encyclopedia_sft_path = r"C:\Users\Admin\epub2dataset_data\encyclopedia_sft.jsonl"
train_tasks = [
    CustomJSON(filepath=encyclopedia_sft_path),  # 2435 rows of encyclopedia
]
train_dataset = TaskMixture(train_tasks)
print0(f"Training mixture: {len(train_dataset):,} rows (MMLU x{args.mmlu_epochs}, GSM8K x{args.gsm8k_epochs})")
val_dataset = TaskMixture([
    CustomJSON(filepath=encyclopedia_sft_path),
]) # total: 2435 rows
